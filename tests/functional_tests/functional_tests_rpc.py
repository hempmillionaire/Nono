...
# a main offline nonod, does most of the tests
# two local online nonods connected to each other
N_MONERODS = 5

# 4 wallets connected to the main offline nonod
# 1 wallet connected to the first local online nonod
# 1 offline wallet
N_WALLETS = 7
...
wallet_base = [builddir + "/bin/nono-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1", "--allow-mismatched-daemon-version"]
wallet_extra = [
...
for i in range(N_MONERODS):
  ...
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/nonod' + str(i) + '.log', 'a+'))
